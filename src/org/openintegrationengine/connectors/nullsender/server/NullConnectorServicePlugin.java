 ...
 * happily <em>writes</em> a channel using the Null Sender and then cannot read it back:
 * {@code PUT /channels/{id}} answers {@code 200}, the channel is stored as Mirth's
 * {@code InvalidChannel}, it never appears on the dashboard, and nothing anywhere says why.
 * That is the failure mode {@code scripts/oie-config-push.sh} exists to catch, and it is
 * what any third-party connector hits the first time it is deployed.
 *
 ...
 */
public class NullConnectorServicePlugin implements ServicePlugin {

    public static final String PLUGIN_POINT_NAME = "Null Connector";

    /** A wildcard, so the AckMode enum is covered along with the properties class. */
    private static final String ALLOWED_PACKAGE = "org.openintegrationengine.connectors.nullsender.**";

    private static final Logger logger = LogManager.getLogger(NullConnectorServicePlugin.class);
    ...
    private void allowTypes() {
        try {
            ...
            logger.error("Could not register the Null connector's classes with the channel"
                    + " serializer. Channels using the Null Sender will be stored as invalid"
    ...
    /** No service of its own, so nothing to guard with a permission. */
    @Override
